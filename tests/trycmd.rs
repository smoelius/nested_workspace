...
/// wildcard after `running 1 test` unless the individual test's result line follows immediately
/// so slow runners do not make the snapshot flaky. Trycmd's multiline wildcard normalization
/// otherwise reports the final blank line as extra when the notification wildcard matches no
/// lines, so require a trailing wildcard as well.
