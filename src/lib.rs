...
/// Creates a builder for running `cargo check` or `cargo build` on nested workspaces.
///
/// Runs `cargo check` when the parent Cargo command is `cargo check`, and `cargo build` otherwise.
#[must_use]
...
impl Builder {
    ...
    ///
    /// The argument is passed unchanged, after automatically prepended arguments and before
    /// arguments inherited from the parent Cargo invocation.
    #[must_use]
    ...
    ///
    /// Arguments are passed unchanged, after automatically prepended arguments and before
    /// arguments inherited from the parent Cargo invocation.
    #[must_use]
